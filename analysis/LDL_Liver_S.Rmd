...
load(paste0(results_dir, "/", analysis_id, "_ctwas_z_snp.Rd"))
...
ctwas_gene_res$genename <- intron_info[sapply(ctwas_gene_res$id, match, intron_info$gene), "genename"]
...
```{r}
df_gene_S <- aggregate(ctwas_gene_res$susie_pip,by=list(ctwas_gene_res$genename), FUN=sum)
...
head(df_gene_S[order(-df_gene_S$susie_pip),], max(sum(df_gene_S$susie_pip>0.8), 20))
```
...
```{r}
ctwas_gene_res[ctwas_gene_res$genename=="CNIH4",]
```

```{r}
ctwas_gene_res[ctwas_gene_res$genename=="FKRP",]
```

```{r}
ctwas_gene_res[ctwas_gene_res$genename=="CCDC57",]
```
...
locus_plot_final_pub <- function(region_tag, xlim=NULL, return_table=F, focus=NULL, label_panel="TWAS", label_genes=NULL, label_pos=NULL, plot_eqtl=NULL, rerun_ctwas=F, rerun_load_only=F, legend_side="right", legend_panel="cTWAS", twas_ymax=NULL){
  ...
  #      paste0("sQTL ", length(eqtl_pos)), srt = 0, pos = 2, xpd = TRUE, cex=0.7)
  # 
...
```{r}
a <- locus_plot_final_pub(region_tag="19_33", return_table=T,
                      focus=NULL,
                      label_genes=NULL,
                      rerun_ctwas=F,
                      rerun_load_only=F,
