...
ctwas_gene_res$chr <- NA
ctwas_gene_res$start.pos <- NA
ctwas_gene_res$end.pos <- NA
...
for(i in 1:dim(ctwas_gene_res)[1]){
  ...
  ctwas_gene_res[i,"chr"] = as.numeric(info[2])
  ctwas_gene_res[i,"start.pos"] = as.numeric(info[3])
  ctwas_gene_res[i,"end.pos"] = as.numeric(info[4])
}

```

```{r}
loop_index <- seq(1,dim(ctwas_gene_res)[1],800)
loop_index <- append(loop_index,dim(ctwas_gene_res)[1]+1)

gene_info <- data.frame()
for(i in 1:(length(loop_index)-1)){
  tmp <- ctwas_gene_res[loop_index[i]:(loop_index[i+1]-1),]
  grf <- GRangesFilter(GRanges(tmp$chr, ranges = IRanges(tmp$start.pos, tmp$end.pos)), type = "any")
  gene_info <- rbind(gene_info,data.frame(genes(edb, filter = grf)))
}
```

```{r}
for(i in 1:dim(gene_info)[1]){
  ctwas_gene_res[which(ctwas_gene_res$start.pos>=gene_info$start[i] & ctwas_gene_res$end.pos<=gene_info$end[i] & ctwas_gene_res$chr==gene_info$seqnames[i]),"genename"] <- gene_info$gene_name[i]
  ctwas_gene_res[which(ctwas_gene_res$start.pos>=gene_info$start[i] & ctwas_gene_res$end.pos<=gene_info$end[i] & ctwas_gene_res$chr==gene_info$seqnames[i]),"id"] <- gene_info$gene_id[i]
  ctwas_gene_res[which(ctwas_gene_res$start.pos>=gene_info$start[i] & ctwas_gene_res$end.pos<=gene_info$end[i] & ctwas_gene_res$chr==gene_info$seqnames[i]),"genename"] <- gene_info$gene_biotype[i]
}
```


```{r}
tt <- head(ctwas_gene_res,100)
grf <- GRangesFilter(GRanges(tt$chr, ranges = IRanges(tt$start.pos, tt$end.pos)), type = "any")
