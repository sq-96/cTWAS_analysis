...
results_dir <- paste0("/project2/xinhe/shengqian/cTWAS/cTWAS_analysis/data/IBD_ME_3kb_0.05/")
...
for index, row in ctwas_gene_res_df.iterrows():
  gene_names = data.gene_names_at_locus(contig=int(row["chrom"]), position=int(row["methylation_pos"])-1000,end=int(row["methylation_pos"])+1000)
  if len(gene_names)!=0:
    for i in gene_names:
      if i != '':
...
ctwas_gene_res = ctwas_gene_res[,!(colnames(ctwas_gene_res) %in% c("methylation_pos", "methylation_id"))]
...
ctwas_gene_res$num_meqtl <- num_meqtl[ctwas_gene_res$id]
...
locus_plot_final_pub <- function(region_tag, xlim=NULL, return_table=F, focus=NULL, label_panel="TWAS", label_genes=NULL, label_pos=NULL, plot_eqtl=NULL, rerun_ctwas=F, rerun_load_only=F, legend_side="right", legend_panel="cTWAS", twas_ymax=NULL){
  ...
  region <- list()
  region_df <- data.table::fread(paste0(results_dir,analysis_id,"_ctwas_ld_R_chr",as.character(region_tag1),".txt"))
  region_df <- region_df[region_df$region_name==region_tag2,]
  region$start <- region_df$start
  region$stop <- region_df$stop
  region$regRDS <- region_df$RDS_file
  region$R_g_file <- paste0(results_dir,analysis_id,"_ctwas_LDR/chr",as.character(region_tag1),"_reg",as.character(region_tag2),".R_gene.RDS")
  region$R_sg_file <- paste0(results_dir,analysis_id,"_ctwas_LDR/chr",as.character(region_tag1),"_reg",as.character(region_tag2),".R_snp_gene.RDS")
  
  expr_df <- data.table::fread(paste0(results_dir,analysis_id,"_expr_chr",as.character(region_tag1),".exprvar"))
  expr_df <- expr_df[expr_df$p0>=region$start & expr_df$p1<=region$stop,]
  region$gid <- expr_df$id
  
  R_snp_info <- do.call(rbind, lapply(region$regRDS, function(x){data.table::fread(paste0(tools::file_path_sans_ext(x), ".Rvar"))}))
  
  region$sidx <- R_snp_info[R_snp_info$pos>=region$start & R_snp_info$pos<=region$stop,]$id
  ...
  if (label_panel=="TWAS" | label_panel=="both"){
    for (i in 1:length(label_genes)){
      text(a$pos[a$id==label_genes[i]], a$PVALUE[a$id==label_genes[i]], labels=paste0(label_genes," (",a$genename[a$id==label_genes[i]],")"), pos=label_pos[i], cex=0.7)
    }
  }
  ...
  text(start, length(plot_eqtl)-(1:length(plot_eqtl))+0.5,  
       labels = paste0(plot_eqtl, " meQTL ", length(eqtl_pos)), srt = 0, pos = 2, xpd = TRUE, cex=0.7)
  ...
  if (label_panel=="cTWAS" | label_panel=="both"){
    for (i in 1:length(label_genes)){
      text(a$pos[a$id==label_genes[i]], a$susie_pip[a$id==label_genes[i]], labels=paste0(label_genes," (",a$genename[a$id==label_genes[i]],")"), pos=label_pos[i], cex=0.7)
    }
  }
...
```{r}
a <- locus_plot_final_pub(region_tag="5_106", return_table=T,
                      focus=NULL,
                      label_genes=NULL,
                      rerun_ctwas=F,
                      rerun_load_only=F,
