...
head(ctwas_gene_res[order(-ctwas_gene_res$susie_pip),c("genename","gene_id","susie_pip","group")], max(sum(ctwas_gene_res$susie_pip>0.8), 20))
...
#df_gene$expression_pip <- round(unlist(sapply(df_gene$genename, function(x){out <- ctwas_gene_res$susie_pip[ctwas_gene_res$genename==x & ctwas_gene_res$group=="Expression"]; if (length(out)==0){out <- 0}; out})),3)

#df_gene$splicing_pip <- round(unlist(sapply(df_gene$genename, function(x){out <- ctwas_gene_res$susie_pip[ctwas_gene_res$genename==x & ctwas_gene_res$group=="Splicing"]; if (length(out)==0){out <- 0}; out})),3)

#df_gene$methylation_pip <- round(unlist(sapply(df_gene$genename, function(x){out <- ctwas_gene_res$susie_pip[ctwas_gene_res$genename==x & ctwas_gene_res$group=="Methylation"]; if (length(out)==0){out <- 0}; out})),3)
...
#df_gene <- merge(df_gene,ctwas_gene_res_M,by.y = "genename",all.x = T)
...
#df_gene$methylation_only_pip <- round(df_gene$methylation_only_pip,3)
...
genes <- df_gene$genename[df_gene$combined_pip>1]
```
