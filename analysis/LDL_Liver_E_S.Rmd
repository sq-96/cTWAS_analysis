...
df$group[df$group=="Liver"] <- "Expression"
df$group[df$group=="Liver_Splicing"] <- "Splicing"
#df$group[df$group=="mashr_Liver_Splicing"] <- "Methylation"
...
ctwas_gene_res$group[ctwas_gene_res$group=="Liver"] <- "Expression"
ctwas_gene_res$group[ctwas_gene_res$group=="Liver_Splicing"] <- "Splicing"
...
df_gene <- rbind(df_gene_E,df_gene_S)
...
```{r echo=FALSE}
pdf(file = "./LDL_LDLR_locus1.pdf", width = 5, height = 3.5)
...
```{r}
pdf(file = "./LDL_LDLR_genetrack.pdf", width = 3.86, height = 0.2)
locus_plot_gene_track_pub(19, 1.1e+07, 11169507, label_pos="above")
dev.off()
```


```{r eval=FALSE}
pdf(file = "./hist.pdf", width = 4, height = 5)

library(RColorBrewer)
coul <- brewer.pal(5, "Set2") 

data <- data.frame(
  name=c("Expression","Expression+Splicing"),
  value=c(34,70)
)

barplot(c(0,0), ylim=c(0,100), names=data$name, ylab="Number of Genes with PIP > 0.8",width=c(0.5,0.5),col=coul)
...
xx <- barplot(data$value, ylim=c(0,100), names=data$name, ylab="Number of Genes with PIP > 0.8",width=c(0.5,0.5),col=coul,add=T)

text(x = xx, y = rep(0, length(c(34,70))), label = paste0(c(34,70), " detected"), pos = 3, cex=0.8)
dev.off()
```