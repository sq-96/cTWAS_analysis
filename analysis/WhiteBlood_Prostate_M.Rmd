...
params:
  analysis_id: "White_Blood_Prostate"
  trait_id: "White_Blood_M"
  weight: "Prostate"
---
...
db = RSQLite::dbConnect(sqlite, "/project2/xinhe/shengqian/cTWAS/cTWAS_analysis/data/mqtl/Prostate.db")
query <- function(...) RSQLite::dbGetQuery(db, ...)
gene_info <- query("select gene, genename, genename_HGNC from extra")
...
ctwas_gene_res <- cbind(ctwas_gene_res, gene_info[sapply(ctwas_gene_res$id, match, gene_info$gene), c("genename","genename_HGNC")])


ctwas_gene_res <- ctwas_gene_res[,colnames(ctwas_gene_res)[!(colnames(ctwas_gene_res) %in% c("genename_HGNC"))]]
...
report_cols <- colnames(ctwas_gene_res)[!(colnames(ctwas_gene_res) %in% c("type", "region_tag1", "region_tag2", "cs_index", "z_flag", "chrom", "pos"))]
